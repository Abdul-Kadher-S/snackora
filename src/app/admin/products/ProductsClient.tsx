...
const PRESET_FOOD_IMAGES = [
  ...
  { label: 'Brownie & Lava Cake', url: 'https://images.unsplash.com/photo-1606313564200-e75d5e30476c?w=600&auto=format&fit=crop&q=80' },
  ...
  { label: 'Combo Box', url: 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=600&auto=format&fit=crop&q=80' },
];
...
export function ProductsClient() {
  ...
  const handleToggleAvailability = async (product: Product) => {
    try {
      ...
      if (res.ok) {
        showToast(
          `${product.name} is now ${!product.available ? 'AVAILABLE' : 'DISABLED'}`,
          'info'
        );
        loadData();
      }
  ...
  const handleToggleSnackpoints = async (product: Product) => {
    try {
      const newState = !product.earnSnackpoints;
      const res = await fetch(`/api/products/${product.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ earnSnackpoints: newState }),
      });

      if (res.ok) {
        showToast(
          `${product.name}: SnackPoints ${newState ? 'ENABLED ⭐' : 'DISABLED'}`,
          'success'
        );
        loadData();
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Add, edit, price, discount, and toggle SnackPoints for products live in the store.
          </p>
        </div>
      ...
      <div className="bg-slate-800/90 border border-slate-700/80 rounded-2xl p-4 flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by snack name, brand, or keyword..."
                ...
                filteredProducts.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-700/30 transition">
                      ...
                      {/* Category */}
                      <td className="p-3.5">
                        <span className="bg-slate-900 text-slate-300 font-semibold px-2 py-1 rounded-lg text-[11px] border border-slate-700">
                      ...
                      <td className="p-3.5">
                        <div className="flex items-baseline gap-1.5">
                          <Currency
                            amount={p.price}
                            showOriginal={p.originalPrice}
                            className="font-black text-white text-sm"
                          />
                        </div>
                        {p.discount > 0 ? (
                          <span className="inline-block mt-0.5 px-2 py-0.5 bg-emerald-950/80 border border-emerald-800 text-emerald-400 rounded-md text-[10px] font-bold">
                            {p.discount}% OFF
                          </span>
                        ) : (
                          <span className="text-[10px] text-slate-500">No discount</span>
                        )}
                      </td>

                      {/* SnackPoints 1-Click Toggle */}
                      <td className="p-3.5">
                        <button
                          type="button"
                          onClick={() => handleToggleSnackpoints(p)}
                          className={`px-2.5 py-1 rounded-xl text-xs font-bold transition flex items-center gap-1.5 ${
                            p.earnSnackpoints !== false
                              ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40 hover:bg-amber-500/30'
                              : 'bg-slate-900 text-slate-500 border border-slate-700 hover:bg-slate-800'
                          }`}
                          title="Click to toggle SnackPoints earning on this snack"
                        >
                          <Star className={`w-3.5 h-3.5 ${p.earnSnackpoints !== false ? 'fill-amber-400 text-amber-400' : 'text-slate-500'}`} />
                          <span>{p.earnSnackpoints !== false ? 'Enabled' : 'Disabled'}</span>
                        </button>
                      </td>
                      ...
                      <td className="p-3.5 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            ...
                            title="Edit product, pricing, or discount"
                          >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-6 bg-slate-950/80 backdrop-blur-sm animate-fade-in overflow-y-auto">
          <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden my-auto animate-scale-up text-white">
            {/* Modal Header */}
            <div className="p-4 md:p-6 border-b border-slate-800 flex items-center justify-between bg-slate-950">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Update price, discount, SnackPoints, image, and descriptions
                </p>
              </div>
              <button
            ...
            {/* Modal Form */}
            <form onSubmit={handleSaveProduct} className="p-4 md:p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              {formError && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#FF6B00]/50 text-white"
                  >
                    {categories.map((c) => (
                      <option key={c.id} value={c.id}>
                        {c.name}
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2 bg-slate-800 border border-slate-700 rounded-xl text-sm font-bold text-white focus:outline-none focus:ring-2 focus:ring-[#FF6B00]/50"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. 20"
                      className="w-full px-3.5 py-2 bg-slate-800 border border-slate-700 rounded-xl text-sm text-emerald-400 font-bold focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
                    />
                  </div>
                </div>
              </div>

              {/* SnackPoints, Trending, Half-Price Toggles */}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 flex items-center justify-between">
                  <span>Image URL (or Click Quick Preset) *</span>
                </label>
                <div className="flex gap-2 mb-2">
                  <input
                    ...
                    placeholder="https://..."
                    className="flex-1 px-3.5 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-[#FF6B00]/50"
                  />
                </div>
                {/* Presets */}
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:outline-none"
                  >
                    <option value="INDIAN">🇮🇳 Desi (Indian)</option>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. midnight, maggi, spicy, combo, drinks"
                  className="w-full px-3.5 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:outline-none focus:ring-2 focus:ring-[#FF6B00]/50"
                />
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                />
                <label htmlFor="available" className="text-xs font-bold text-slate-300">
              ...
              <div className="pt-4 border-t border-slate-800 flex justify-end gap-2.5">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="px-6 py-2 bg-[#FF6B00] hover:bg-[#EA580C] disabled:bg-slate-700 text-white font-bold rounded-xl text-xs transition shadow-md shadow-orange-500/20 flex items-center gap-2"
                >
                  {saving ? (
                    <>
